...
for file in pbar:
    ...
    plt.savefig(f'./animations/frame_{i}.png', transparent=True,
                dpi=100, format='png', facecolor='white', bbox_inches='tight')
    i += 1
    plt.close()

print("vGRF_L = {:.2f}, vGRF_R = {:.2f}  ".format(torch.stack(
    vGRF_L).mean().item(), torch.stack(vGRF_R).mean().item()))
print("vRPE = {:.2f}, ".format(torch.stack(vRPE).mean().item()))

with open('./animations/vGRF_values.txt', 'w') as f:
    f.write("vGRF_L: {:.2f}\n".format(torch.stack(vGRF_L).mean().item()))
    f.write("vGRF_R: {:.2f}\n".format(torch.stack(vGRF_R).mean().item()))
    # if not wild[subj]:
